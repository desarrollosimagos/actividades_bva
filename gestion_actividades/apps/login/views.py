...
from apps.empleados.tareas.models import Empleados
...
class RegistrarseUsuario(CreateView):
    ...
    def post(self, request, *args, **kwargs):
       ...
       if accion == "buscar":
            ...
            if existe:
                ...
                # agregar al Modelo UsuarioDepart las relaciones del usuario django
                usuariodepart = UsuarioDepart()
                
                
                print "GGGGG",departamento_id
                usuariodepart.grupo_usuario = grupo_id
                usuariodepart.departamento_id = departamento_id
                usuariodepart.usuario_id = ultimo_id_user
                usuariodepart.save()
            ...
            user_id = request.POST.get('valor')
            for g in grupo:

                usuariodepart.grupo_usuario = g
                usuariodepart.departamento_id = departamento_id
                usuariodepart.usuario_id = user_id
                usuariodepart.save()
    ...
    def get_context_data(self, **kwargs):
        ...
        datos_v = datos.values('id', 'nom_departamento')
...
class EliminarUsuarios(DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        existe = UsuarioDepart.objects.filter(usuario_id=self.kwargs['pk'])
        ...
        id_departamento = existe.values('departamento_id')
        
        if tipo_grupo == 'gerente':
            actividades = Actividad.objects.filter(departamento_id=id_departamento)
        ...
        elif tipo_grupo == 'usuario':
            actividades = Empleados.objects.filter(empleados_id=self.kwargs['pk'])
...
def buscar_grupos(request):
    ...
    ##obj = UsuarioDepart.objects.get(usuario_id=id_user).departamento_id
...
def buscar_departamentos(request):
    ...
    existe = UsuarioDepart.objects.filter(usuario_id=id_user).exists()
    if existe:
        
        obj = UsuarioDepart.objects.get(usuario_id=id_user).departamento_id
        #print obj
        
        lista_depart.append(obj)
        
        data = json.dumps(lista_depart)
